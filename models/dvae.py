...
def compute_loss(loss_1, loss_2, start,target,ntime, niter):
    '''
...
class Group(nn.Module):
    ...
    def forward(self, xyz):
        ...
        # fps the centers out
        center = pc.fps(xyz, self.num_group) # B G 3
...
class DiscreteVAE(LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        points = data  # b n 3
        ret = self(points)
        loss_1, loss_2 = self.get_loss(ret, points)
        niter = self.current_epoch * self.trainer.num_training_batches + batch_idx
        loss = compute_loss(loss_1, loss_2, start=0, target=0.1, ntime=100000, niter=niter)
        self.log("train_loss", loss, on_step=True, on_epoch=True, sync_dist=True, prog_bar=True, logger=True)
        self.log("cd_loss", loss_1, on_step=True, on_epoch=True, sync_dist=True, prog_bar=True, logger=True)
        self.log("kld_loss", loss_2, on_step=True, on_epoch=True, sync_dist=True, prog_bar=True, logger=True)
        return loss
